...
// Start coding here
let locArr = bills.map((customer) => customer.location);
let totArr = bills.map((customer) => customer.total);

function convertToObject(k, v) {
  let obj3 = {};
  let length = Math.min(k.length, v.length);

  for (let i = 0; i < length; i++) {
    if (obj3[k[i]]) {
      obj3[k[i]] = obj3[k[i]] + v[i];
    } else {
      obj3[k[i]] = v[i];
    }
  }
...
const totalPaidByLocation = convertToObject(locArr, totArr);
